...
export default function Component() {
  ...
  useEffect(() => {
    ...
    dataService.getCurrencies().then(currencies => {
      ...
      setSelectedCurrency(
        currencies.find(c => c.name === 'CUP') || currencies[0]
      )
    })
  }, [])
  ...
  const calculateNetProfit = (orders: Order[]) => {
    return orders.reduce((total, order) => {
      const orderTotal = convertCurrency(
        order.amount_paid,
        order.items[0].variation.currency,
        selectedCurrency!
      )
      const suppliesCost = order.items.reduce((itemTotal, item) => {
        return (
          itemTotal +
          item.variation.supplies.reduce((supplyTotal, supply) => {
            return (
              supplyTotal +
              convertCurrency(supply.cost, supply.currency, selectedCurrency!)
            )
          }, 0)
        )
      }, 0)
    ...
    <div className='container mx-auto p-4'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-8'>
        ...
        <div className='p-4 bg-blue-100 rounded'>
          ...
          <p className='text-2xl'>
            {formatCurrency(programmerPay, selectedCurrency)}
          </p>
        </div>
      </div>
              ...
              return (
                <React.Fragment key={order.id}>
                  <tr className='hover:bg-gray-50'>
                    ...
                    <td className='py-2 px-4 border-b'>
                      {formatCurrency(suppliesCost, selectedCurrency)}
                    </td>
                    <td className='py-2 px-4 border-b'>
                                      ...
                                      {index === 0 && (
                                        <>
                                          <td
                                            ...
                                            rowSpan={
                                              item.variation.supplies.length
                                            }
                                          >
                                            {item.variation.name}
                                          </td>
